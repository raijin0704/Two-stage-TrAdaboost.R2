...
class SimpleTrAdaBoostRT:
    def __init__(self,
        ...
        # thresholdが所与の値であるかどうかを判定して保存しておく
        if threshold is None:
            self.threshold_given_F = False
        else:
            self.threshold_given_F = True
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for iboost in range(self.n_estimators): # this for loop is sequential and does not support parallel(revison is needed if making parallel)
            # Boosting step
            sample_weight, estimator_weight, estimator_error = self._simple_TrAdaboostRT(
                    iboost,
                    X, y,
                    sample_weight)
            # Early termination
    ...
    def _simple_TrAdaboostRT(self, iboost, X, y, sample_weight):
        ...
        if not iboost == self.n_estimators - 1:
            ...
            if self.threshold_given_F == False:
                ...
                threshold = np.median(error_vect)
                sample_weight[:-self.sample_size[-1]] *= np.where( error_vect[:-self.sample_size[-1]] > threshold, 0,
                        np.power(
                        beta,
                        -(1. - error_vect[:-self.sample_size[-1]]) * self.learning_rate))
...
#         self.models_ = []
#         self.errors_ = []
#         for istep in range(self.n_estimators):
#             # 普通にAdaBoost.R2にかける
